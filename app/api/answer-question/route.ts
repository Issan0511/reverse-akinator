...
export async function POST(request: NextRequest) {
  try {
    ...
    const payload = {
      ...
      以下のデータは、アキネーターゲームのお題(charactername)と質問です。
      答えに至る思考過程も出力したのち、
      以下の質問を"answer":{"はい"||"いいえ"||"部分的にはい"||"分からない"||"答えに到達"}に分類することが目標です。
      質問に対する答えはできる限り論理的にかつ日本語的に正しくあることを心掛けて下さい。
      真偽が不明な質問、あなたがその質問に確実に答える知識をもっていない質問、"はい"||"いいえ"で回答できない質問については"分からない"と出力して下さい
      もしお題に対しての質問が、一般的には間違っているが広い意味で正しい場合や、正しい場合と間違っている場合が両方存在する場合は"部分的にはい"と出力してください。
      もし'答えは"charactername"ですか？'や、'"charactername"？'のように答えが明確な質問があり、かつその答えがキャラクターの名前である場合は"答えに到達"を出力してください。
      ...
      answer = {
        ...
        "judgement": "string" // {"はい"||"いいえ"||"部分的にはい"||"分からない"||"答えに到達"} のいずれか
      }
      Result: answer          
          `
        }]
